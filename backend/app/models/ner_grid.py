...
class NerGrid(Base):
    ...
    aspect_sin: Mapped[float] = mapped_column(Float)
    aspect_cos: Mapped[float] = mapped_column(Float)

    historical_landslides: Mapped[float] = mapped_column(Float)

    geom: Mapped[object] = mapped_column(
        Geometry("POLYGON", srid=7771)
    )